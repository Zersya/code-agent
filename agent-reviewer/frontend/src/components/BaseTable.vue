            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <template v-if="column.sortable">
                  <svg v-if="sortBy === column.key && sortOrder === 'asc'" class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                  </svg>
                  <svg v-else-if="sortBy === column.key && sortOrder === 'desc'" class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <tr v-else>
            <td :colspan="columns.length" class="px-6 py-12 text-center text-sm text-gray-500">
              <div class="flex flex-col items-center">
                ...
                <p class="text-gray-500">{{ emptyMessage || 'No records found' }}</p>
              </div>
            </td>
          </tr>
        </tbody>
